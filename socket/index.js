...
function getRandomClient(users, hashString) {
  const randomIndex = parseInt(
    (parseInt(hashString) || hashString.charCodeAt(0) || hashString.length) %
      users.length
  );
  ...
  io.of("/").on(CONNECTION, socket => {
    ...
    socket.on(USER_LOGOUT, () => {
      if (onUsers.map(client => client.socketId).includes(socket.id)) {
        onUsers.splice(
          _.findIndex(onUsers, client => client.socketId === socket.id),
          1
        );
      }
    });
    ...
    socket.on(FIND_MATCH_PARTNER, (hostUser, prevCombatRoomKey) => {
      const hostUserIndex = _.findIndex(
        onUsers,
      ...
      if (onUsers.length) {
        ...
        io.of("/")
          .to(guestSocketId)
          .emit(SEND_MATCH_INVITATION, hostUser, combatRoomKey);
        io.of("/")
          .to(combatRoomKey)
          .emit(PENDING_MATCH_ACCEPTANCE, guestUser);
      } else {
        io.of("/")
          .to(combatRoomKey)
          .emit(MATCH_PARTNER_UNAVAILABLE);
    ...
    socket.on(ACCEPT_MATCH_INVITATION, (combatRoomKey, matchPartner) => {
      ...
      io.of("/")
        .to(combatRoomKey)
        .emit(MATCH_PARTNER_ENTERED, matchPartner, combatRoomKey);
    });

    socket.on(REFUSE_MATCH_INVITATION, (combatRoomKey, guestUser) => {
      io.of("/")
        .to(combatRoomKey)
        .emit(MATCH_PARTNER_REFUSE_MATCH_INVITATION, combatRoomKey);
    });
    ...
    socket.on(SEND_RANDOM_PROBLEM, (problem, combatRoomKey, matchId) => {
      const time = problem.difficulty_level * 30 * 60 * 1000;
      const min = 60 * 1000;
      ...
      if (time <= min * 3) {
        const secondIntervalId = setInterval(() => {
          io.of("/")
            .to(combatRoomKey)
            .emit(MATCH_TIMER, time);
          if (time < 1000) {
            io.of("/")
              .to(combatRoomKey)
              .emit(MATCH_TIMER, time); // 게임 종료
      ...
      io.of("/")
        .to(combatRoomKey)
        .emit(MATCH_START, problem, matchId, time);
    });

    socket.on(KEY_DOWN, combatRoomKey => {
      socket.broadcast
        .to(combatRoomKey)
        .emit(MATCH_PARTNER_KEY_DOWN, combatRoomKey);
    });

    socket.on(KEY_UP, combatRoomKey => {
      socket.broadcast
        .to(combatRoomKey)
        .emit(MATCH_PARTNER_KEY_UP, combatRoomKey);
    });

    socket.on(
      SOLUTION_SUBMITTED,
      (testResult, countPassed, isPassedAll, combatRoomKey) => {
        socket.broadcast
          .to(combatRoomKey)
          .emit(
            MATCH_PARTNER_SOLUTION_SUBMITTED,
    ...
    socket.on(USER_WINNING, (matchResult, combatRoomKey) => {
      socket.broadcast
        .to(combatRoomKey)
        .emit(MATCH_PARTNER_WINNING, matchResult);
      socket.leave(combatRoomKey);
    });
